...
function App() {
  ...
  return (
    <div>
      ...
      {collections ? (
        <ul>
          {nftData && nftData.collections.map((collection, index) => (
  <li key={index}>
    ...
    <strong>Floor Price:</strong> {collection.floor_prices?.[0]?.value ? `${collection.floor_prices[0].value / 1e9} SOL (USD ${collection.floor_prices[0].value_usd_cents / 100})` : 'N/A'}<br />
