...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchBirdeyeData = async () => {
      ...
      try {
        ...
        const data = await response.json();
        console.log('Birdeye Data:', data); // Log the data to the console
        setBirdeyeData(data);
      } catch (error) {
  ...
  return (
    <div className="NFTs">
      ...
      {birdeyeData ? (
  <div>
    <h2>Birdeye Token Information</h2>
    <ul>
      <li>
        <strong>Name:</strong> {birdeyeData.name || 'N/A'} <br />
        <strong>Symbol:</strong> {birdeyeData.symbol || 'N/A'} <br />
        <strong>Decimals:</strong> {birdeyeData.decimals || 'N/A'} <br />
        {/* Include additional properties as needed */}
      </li>
    </ul>
  </div>
) : (
  <p>Birdeye data is not available or is not in the expected format.</p>
)}
    </div>
  );
}
