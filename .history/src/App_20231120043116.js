        ...
        <ul>
          {collections.map((collection, index) => (
            <li key={index}>
              ...
              <strong>Floor Price (Tensor, SOL):</strong> {
  collection.floor_prices && collection.floor_prices.length > 0
    ? collection.floor_prices.some(price => price.marketplace_id === 'tensor')
      ? `${collection.floor_prices.find(price => price.marketplace_id === 'tensor').value / 1e9} SOL`
      : 'N/A'
    : 'N/A'
}<br />
