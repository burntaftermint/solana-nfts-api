...
function App() {
  ...
  <li key={index}>
    ...
    <strong>Description:</strong> {collection.collection_details?.description || 'N/A'}<br />
<strong>Floor Price (Magic Eden, SOL):</strong> {console.log(collection.floor_prices)} {collection.floor_prices?.find(price => price.marketplace_id === 'magiceden') ? collection.floor_prices.find(price => price.marketplace_id === 'magiceden').value / 1e9 : 'N/A'} SOL<br />
<strong>Floor Price (Tensor, SOL):</strong> {console.log(collection.floor_prices)} {collection.floor_prices?.find(price => price.marketplace_id === 'tensor') ? collection.floor_prices.find(price => price.marketplace_id === 'tensor').value / 1e9 : 'N/A'} SOL<br />
<strong>24-hour Volume:</strong> {collection.volume_string ? `${parseFloat(collection.volume_string) / 1e9} SOL` : 'N/A'}<br />
