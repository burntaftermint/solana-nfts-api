        ...
        <ul>
          {collections.map((collection, index) => (
            <li key={index}>
              ...
              <strong>Floor Price:</strong> {collection.floor_prices?.[0]?.value !== undefined ? `${collection.floor_prices[0].value / 1e9} SOL` : {collection.floor_prices?.[0]?.value !== undefined ? `${collection.floor_prices[0].value / 1e9}<br />
